...
if (require.main === module) {
	...
	if (process.argv[2]) {
		...
		fs.writeFile(__dirname + '/alchemy_key.txt',process.argv[2], function(err) {
			if (err) {
				console.log('Error, unable to write key file: ' + err);
				process.exit(1);
			} else {
				console.log('AlchemyAPI key: ' + process.argv[2] + ' successfully written to alchemy_key.txt');
				process.exit(0);
			}
		});
	} else {
...
function AlchemyAPI() {
	...
	//Load the key from alchemy_key.txt
	try {
		key = fs.readFileSync(__dirname + '/alchemy_key.txt').toString().trim();
	}
	catch(err) {
		//Create the file
		fs.openSync(__dirname + '/alchemy_key.txt', 'w');
		console.log('API key not detected in alchemy_key.txt, please run: node alchemyapi.js YOUR_KEY_HERE');
	...
	if (key.length != 40) {
		console.log('The API key in alchemy_key.txt does not appear to be valid. Make sure to run: node alchemyapi.js YOUR_KEY_HERE');
		console.log('If you do not have a key, register for one at: http://www.alchemyapi.com/api/register.html');
		process.exit(1);
	}
